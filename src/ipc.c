...
ipc_t* ipc_init_module(const char *ipc_name, ngx_cycle_t *cycle) {
    ...
    ngx_int_t max_processes = ccf->worker_processes;
    size_t process_slots_sz = sizeof(process_slot_tracking_t) * max_processes;
    ipc_shm_data_t *shdata = NULL;
    ...
    shdata->process_count = max_processes;
...
ngx_int_t ipc_init_worker(ipc_t* ipc, ngx_cycle_t* cycle) {
    ...
    process_slot_tracking_t* procslot = NULL;
    ...
    procslot = &shdata->process_slots[ngx_worker];
    procslot->pid = ngx_pid;
    procslot->slot = ngx_worker;
    procslot->ngx_process_type = ngx_process;
    DBG("ADD  process %i slot %i type %i", ngx_pid, ngx_worker, ngx_process);
...
ngx_pid_t ipc_get_pid(ipc_t *ipc, ngx_uint_t process_slot) {
    ipc_shm_data_t* shdata = ipc->shm;
    process_slot_tracking_t *process_slots = shdata->process_slots;
    int slot = process_slot;

    if (slot >= 0 && slot < shdata->process_count &&
            process_slots[process_slot].slot == process_slot &&
            process_slots[process_slot].pid > 0) {
        return process_slots[process_slot].pid;
    }

    return NGX_INVALID_PID;
}
ngx_uint_t ipc_get_slot(ipc_t* ipc, ngx_pid_t pid) {
    ipc_shm_data_t* shdata = ipc->shm;
...
static ngx_int_t ipc_activate_channel(ipc_t* ipc, ngx_cycle_t* cycle,
                                      ipc_channel_t* channel,
                                      ipc_socket_type_t socktype) {
    int rc = NGX_OK;
...
static ngx_int_t ipc_open(ipc_t* ipc, ngx_cycle_t* cycle, ngx_int_t workers) {
    ...
    */
    for (i=0; i < workers; ++i) { //workers and possible cache loader and manager
        channel = &ipc->channel[i];
        if (ipc_activate_channel(ipc, cycle, channel, IPC_PIPE) != NGX_OK) {
            return NGX_ERROR;
        }

        shdata->process_slots[i].slot = i;
...
static ngx_int_t ipc_close_channel(ipc_channel_t *chan) {
    ipc_alert_link_t* cur = NULL;
    ipc_alert_link_t* cur_next = NULL;
    ipc_readbuf_t* rcur = NULL;
...
static ngx_int_t ipc_enqueue_tmp_iovec(ipc_t* ipc, ipc_writebuf_t* wb) {
    ...
    link = malloc(sizeof(ipc_alert_link_t) + sz);
...
static ngx_int_t ipc_channel_setup_conn(ipc_channel_t* chan,
                                        ngx_cycle_t* cycle,
                                        ipc_conn_type_t conn_type,
                                        void (*event_handler)(ngx_event_t*),
                                        void* data) {
...
static ngx_int_t ipc_read(ipc_t* ipc, ipc_channel_t* ipc_channel, ipc_alert_handler_pt handler, ngx_log_t* log) {
    ...
    while (1) {
        ...
        case '$':
            ...
            handler(ipc, header.src_pid, header.src_slot, &name, &data);
            break;
...
static void ipc_worker_read_handler(ngx_event_t *ev) {
    ...
    c = ev->data;
    ipc = c->data;
    ipc_channel = &ipc->channel[ngx_worker];
...
ngx_int_t ipc_alert_slot(ipc_t* ipc, ngx_uint_t slot, ngx_str_t* name, ngx_str_t* data) {
    DBG("send alert '%V' to slot %i", name, slot);

    ngx_str_t empty = {0, NULL};
    if (!name) name = &empty;
    if (!data) data = &empty;
    if (slot >= NGX_MAX_PROCESSES) return NGX_ERROR;
    if (slot == ngx_worker) {
        ipc->alert_handler(ipc, ngx_pid, slot, name, data);
        return NGX_OK;
    }
...
ngx_int_t ipc_alert_pid(ipc_t* ipc, ngx_pid_t worker_pid, ngx_str_t* name, ngx_str_t* data) {
    ngx_uint_t slot = ipc_get_slot(ipc, worker_pid);
...
#define COLLECT_PROCESS_PROPERTY(shdata, prop, dst_array, found_count) \
...
ngx_pid_t* ipc_get_worker_pids(ipc_t* ipc, int* pid_count) {
    static ngx_pid_t pid_array[NGX_MAX_PROCESSES];
    ipc_shm_data_t* shdata = ipc->shm;
    *pid_count = 0;
    COLLECT_PROCESS_PROPERTY(shdata, pid, pid_array, pid_count)
            return pid_array;
}

ngx_int_t* ipc_get_worker_slots(ipc_t *ipc, int *slot_count) {
    static ngx_int_t slot_array[NGX_MAX_PROCESSES];
    ipc_shm_data_t* shdata = ipc->shm;
    *slot_count = 0;
    COLLECT_PROCESS_PROPERTY(shdata, slot, slot_array, slot_count)
            return slot_array;
}
